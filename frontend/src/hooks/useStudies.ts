...
export const useStudies = (): UseStudiesReturn => {
  ...
  const updateStudy = useCallback(async (id: string, updates: UpdateStudyRequest): Promise<Study | null> => {
    ...
    setError(null);
    try {
      const response: ApiResponse<Study> = await api.put(`/api/studies/${id}`, updates);
      if (response.success && response.data) {
  ...
  const updateStudyStatus = useCallback(async (id: string, status: StudyStatus): Promise<Study | null> => {
    ...
    setError(null);
    try {
      const response: ApiResponse<Study> = await api.patch(`/api/studies/${id}/status`, { status });
      if (response.success && response.data) {
        setStudies(prev => prev.map(s => s.id === id ? response.data! : s));
        if (study?.id === id) {
          setStudy(response.data);
  ...
  const deleteStudy = useCallback(async (id: string): Promise<boolean> => {
    ...
    setError(null);
    try {
      const response: ApiResponse = await api.delete(`/api/studies/${id}`);
      if (response.success) {
  ...
  const fetchStudySessions = useCallback(async (studyId: string) => {
    ...
    setError(null);
    try {
      const response: ApiResponse<StudySession[]> = await api.get(`/api/studies/${studyId}/sessions`);
      if (response.success && response.data) {
  ...
  const fetchStudySession = useCallback(async (sessionId: string) => {
    ...
    setError(null);
    try {
      const response: ApiResponse<StudySession> = await api.get(`/api/studies/sessions/${sessionId}`);
      if (response.success && response.data) {
  ...
  const createStudySession = useCallback(async (sessionData: CreateStudySessionRequest): Promise<StudySession | null> => {
    ...
    setError(null);
    try {
      const response: ApiResponse<StudySession> = await api.post('/api/studies/sessions', sessionData);
      if (response.success && response.data) {
  ...
  const updateStudySession = useCallback(async (sessionId: string, updates: UpdateStudySessionRequest): Promise<StudySession | null> => {
    ...
    setError(null);
    try {
      const response: ApiResponse<StudySession> = await api.put(`/api/studies/sessions/${sessionId}`, updates);
      if (response.success && response.data) {
  ...
  const deleteStudySession = useCallback(async (sessionId: string): Promise<boolean> => {
    ...
    setError(null);
    try {
      const response: ApiResponse = await api.delete(`/api/studies/sessions/${sessionId}`);
      if (response.success) {
  ...
  const fetchStudyResults = useCallback(async (studyId: string) => {
    ...
    setError(null);
    try {
      const response: ApiResponse<StudyResult[]> = await api.get(`/api/studies/${studyId}/results`);
      if (response.success && response.data) {
