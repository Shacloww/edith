...
export const useProtocols = (): UseProtocolsReturn => {
  ...
  const fetchProtocols = useCallback(async () => {
    ...
    setError(null);
    try {
      const response: ApiResponse<Protocol[]> = await api.get('/api/protocols');
  ...
  const fetchProtocol = useCallback(async (id: string) => {
    ...
    setError(null);
    try {
      const response: ApiResponse<Protocol> = await api.get(`/api/protocols/${id}`);
      if (response.success && response.data) {
  ...
  const createProtocol = useCallback(async (protocolData: CreateProtocolRequest): Promise<Protocol | null> => {
    ...
    setError(null);
    try {
      const response: ApiResponse<Protocol> = await api.post('/api/protocols', protocolData);
      if (response.success && response.data) {
  ...
  const updateProtocol = useCallback(async (id: string, updates: Partial<CreateProtocolRequest>): Promise<Protocol | null> => {
    ...
    setError(null);
    try {
      const response: ApiResponse<Protocol> = await api.put(`/api/protocols/${id}`, updates);
      if (response.success && response.data) {
  ...
  const deleteProtocol = useCallback(async (id: string): Promise<boolean> => {
    ...
    setError(null);
    try {
      const response: ApiResponse = await api.delete(`/api/protocols/${id}`);
      if (response.success) {
